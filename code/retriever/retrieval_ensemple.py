...
class TfidfRetriever(_BaseSparseRetriever):
    def __init__(
        self,
        tokenize_fn,
        data_path: Optional[str] = "../data/",
...
class LatentDirichletAllocationRetriever(_BaseSparseRetriever):
    def __init__(
        self,
        tokenize_fn,
        data_path: Optional[str] = "../data/",
...
class FaissRetriever(_BaseSparseRetriever):
    def __init__(
        self,
        tokenize_fn,
        num_clusters=64,
...
class BaseDenseRetriever:
    def __init__(
        ...
        self.model.eval()
        with open(#os.path.join(retriever_path, "encoder_model_name.txt")
                  "/opt/level2_nlp_mrc-nlp-03/retriever_outputs/klue-roberta-base/encoder_model_name.txt", "r") as f:
...
def get_args(method):
    ...
    parser.add_argument("--method", default=method, type=str, help="faiss, tfidf, bm25, dpr")
...
result = [[ensemble[j][i] for j in range(len(ensemble))] for i in range(len(ensemble[0]))]
tray=[]
for i in range(len(result)):
    ...
    for k in range(len(result[0][0])):
        ...
        for p in result[i]:
            put.append(p[k])
        if put[0] == put[1]:
            ksuspect.append(put[0])
        else:
            ksuspect.append(put[-1])
    tray.append(ksuspect)
print(tray)
